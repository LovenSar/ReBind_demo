...
[任务]
1) 对每一行伪代码追加一个简短行尾注释（解释这一行做了什么、关键变量含义、关键副作用）。
2) 空行、仅包含大括号的行（"{{" 或 "}}"）可以不注释。
3) 注释要“贴着代码行”，不要写头部大段注释。
4) 不要编造不存在的系统调用；无法确定时用“疑似/可能”。
5) line_comments 的 value 必须是“纯自然语言注释文本”，不要包含任何代码片段，也不要以 "//" 开头。
6) 每条注释尽量短（建议 <= 30 中文字），不要换行。
...
[任务]
1) 结合反汇编/伪代码，生成若干条简短注释，并用对应的“地址 EA”作为 key。
