...
from .kp_types import UnifiedFunctionNode, UnifiedGraph


logger = logging.getLogger(__name__)


def _build_analyzed_set(
    graph: UnifiedGraph,
    analysis_info: Dict[int, dict],
) -> Set[int]:
    """从分析信息中构建"已分析"函数 entry_va 集合（ANALYZED 或 LOCKED 状态）。"""
...
def compute_unified_scores(
    ...
    *,
    ...
    if analyzed_entry_vas is None:
        analyzed_entry_vas = _build_analyzed_set(graph, analysis_info)
    ...
    for entry_va in entry_vas:
        ...
        if node is None:
            continue
        scores[int(entry_va)] = _score_single_node(node, analyzed_entry_vas, entry_va)
...
def compute_unified_scores_incremental(
    ...
    *,
    ...
    # 受影响集合 = 新分析函数 + 它们的调用者（callees_contrib 会因子函数完成而提升）
    impacted: Set[int] = set(changed_entry_vas)
    ...
    if only_entry_vas is not None:
        allowed = set(only_entry_vas)
        entry_vas_to_compute = [va for va in entry_vas_to_compute if va in allowed]

    for entry_va in entry_vas_to_compute:
        node = graph.nodes.get(int(entry_va))
        if node is None:
            continue
        new_scores[int(entry_va)] = _score_single_node(node, analyzed_entry_vas, entry_va)
