...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    parser.add_argument(
        ...
        default=10,
        help=(
            "Phase 1 中每轮批量评分/LLM 分析的函数数量（默认 10；"
            "设置为 1 则表示每分析一个函数就立刻重算一次评分）。"
        ),
    )
    parser.add_argument(
        "--max-globals",
    ...
    try:
        ...
        batch_size = max(1, int(getattr(args, "phase1_batch_size", 10) or 1))
        ...
        if total_pending == 0:
            ...
            while processed < target_count:
                ...
                # 每个批次开始时重新加载最新分析状态并全量计算评分，
                # 之后在当前批次内只使用这一版评分，避免每个函数都全量重算。
                analysis_info = load_analysis_info(conn)
                scores = compute_unified_scores(unified_graph, analysis_info)
                update_unified_scores_in_db(conn, unified_graph, scores)
                ...
                # 按当前评分从高到低排序，并截取本批次要处理的前若干个函数
                pending_nodes.sort(
                    key=lambda n: scores.get(n.entry_va, 0),
                    reverse=True,
                )
                current_batch_nodes = pending_nodes[: min(len(pending_nodes), batch_size, remaining)]

                for target_node in current_batch_nodes:
                    if processed >= target_count:
                        break
