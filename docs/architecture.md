...
本文描述 ReBind Demo 从初始化到 Phase4 的主流程，与仓库根目录 `rebind_demo.py` 调度的语义对齐阶段一致。
...
更细的执行入口：广度 6 步见 [`tools/Semantics_Alignment/breadth/pipeline.py`](../tools/Semantics_Alignment/breadth/pipeline.py)；深度分析见 [goal-deep-engine.md](./goal-deep-engine.md) 与 `depth/engine.py`。
